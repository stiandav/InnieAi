    ...
    <section className="py-32 bg-surface">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          <p className="text-brand font-semibold text-sm uppercase tracking-widest mb-4">Client Results</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {testimonials.map((t) => {
            ...
            return (
              <div
                key={t.id}
                className="bg-canvas border border-ink/8 rounded-xl p-8 flex flex-col gap-5 hover:border-brand/20 hover:shadow-md transition-all duration-200 shadow-sm"
              >
                {/* Metric callout */}
                {metric && (
                  <div className="inline-flex items-center gap-2 bg-brand-light border border-brand/20 rounded-lg px-4 py-2.5 self-start">
                    ...
                    <span className="text-brand font-semibold text-sm">{metric}</span>
                  </div>
                )}
                ...
                {/* Quote */}
                <blockquote className="font-display text-ink/75 text-base leading-relaxed flex-1 italic">
                  &ldquo;{t.quote}&rdquo;
                </blockquote>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-ink/8">
                  <div>
                    <p className="text-ink font-semibold text-sm">{t.author_name}</p>
                    <p className="text-muted text-xs mt-0.5">{t.author_title}</p>
                  </div>
                  {niche && (
                    <div className="bg-brand-light text-brand text-xs font-medium px-3 py-1 rounded-full border border-brand/15">
                      {niche}
                    </div>
                  )}
                </div>
