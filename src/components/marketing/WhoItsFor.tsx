...
export function WhoItsFor() {
  return (
    <section className="py-32 bg-canvas" id="who">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          <p className="text-brand font-semibold text-sm uppercase tracking-widest mb-4">Built For You</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {niches.map((n) => (
            <Link
              key={n.slug}
              href={`/for/${n.slug}`}
              className="group relative rounded-xl border border-ink/8 bg-surface p-7 flex flex-col gap-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-brand/30 hover:shadow-md shadow-sm"
            >
              <div className="flex items-center justify-between">
                <div className="w-11 h-11 rounded-lg bg-brand-light flex items-center justify-center text-brand group-hover:bg-brand group-hover:text-white transition-all duration-200">
                  {n.icon}
                </div>
                <svg className="w-4 h-4 text-ink/20 group-hover:text-brand/50 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" />
                </svg>
              </div>

              <div>
                <h3 className="font-display text-ink font-semibold text-lg mb-2 tracking-tight">{n.niche}</h3>
                <p className="text-muted text-sm leading-relaxed mb-4">{n.pain}</p>
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-brand text-xs leading-relaxed font-medium">{n.outcome}</p>
                </div>
              </div>

              <div className="mt-auto pt-2 border-t border-ink/6">
                <span className="text-muted text-xs group-hover:text-brand transition-colors">
