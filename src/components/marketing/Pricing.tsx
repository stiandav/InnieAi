    ...
    <section id="pricing" className="py-32 bg-navy-light">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-cream/50 max-w-xl mx-auto mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
          {[
            { label: 'vs Marketing Agency', value: '$5,000–$10,000/mo', sub: 'for similar results' },
            { label: 'vs Hiring In-House', value: '$4,000–$6,000/mo', sub: 'salary + benefits' },
            ...
            <div
              ...
              className={`rounded-xl px-6 py-5 text-center border ${
                item.highlight
                  ...
                  : 'bg-navy border-white/[0.06]'
              }`}
            >
              <p className={`text-xs uppercase tracking-widest font-semibold mb-1 ${item.highlight ? 'text-accent' : 'text-cream/30'}`}>
                {item.label}
              </p>
              <p className={`text-2xl font-bold ${item.highlight ? 'text-cream' : 'text-cream/50 line-through'}`}>
                {item.value}
              </p>
              <p className={`text-xs mt-1 ${item.highlight ? 'text-accent/70' : 'text-cream/30'}`}>{item.sub}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center mb-10">
          <div className="inline-flex items-center gap-1 bg-navy rounded-lg p-1 border border-white/[0.06]">
            <button
              onClick={() => setShowSetup(false)}
              className={`px-5 py-2 rounded-md text-sm font-medium transition-all ${
                !showSetup ? 'bg-accent text-white shadow-sm' : 'text-cream/50 hover:text-cream'
              }`}
            >
              Monthly retainer
            </button>
        ...
        {showSetup && (
          <p className="text-center text-cream/40 text-sm mb-8">
            ...
            >
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 flex items-center gap-2">
                  <div className="bg-accent text-white text-xs font-bold px-4 py-1.5 rounded-full">
                    MOST POPULAR
                  </div>
                  <div className="bg-amber-500/20 border border-amber-500/30 text-amber-300 text-xs font-medium px-3 py-1.5 rounded-full">
                    3 spots left
                  </div>
                </div>
              )}
              ...
              <div className="mb-6 mt-2">
                ...
                <p className="text-cream/50 text-sm mb-5">{plan.description}</p>
                <div className="flex items-baseline gap-1">
                  <span className="text-cream/50 text-lg">$</span>
                  <span className="text-5xl font-bold text-cream tabular-nums">
                ...
                <div className="flex items-center gap-1.5 mt-3">
                  ...
                  <span className="text-green-400/80 text-xs">{plan.roi}</span>
                </div>
              </div>
              ...
              <a
                ...
                className={`w-full text-center py-3.5 rounded-lg font-semibold text-sm transition-all ${
                  plan.popular
                    ? 'bg-accent hover:bg-accent-hover text-white hover:-translate-y-0.5 shadow-lg shadow-accent/20'
